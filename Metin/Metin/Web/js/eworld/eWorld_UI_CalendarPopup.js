...
function getDate(sDate) {
	var sy = sDate.substring(0, 4);
	var sm = sDate.substring(4, 6);
	var sd = sDate.substring(6, 8);
	return new Date(sy, sm - 1, sd);
}

function validateDate(date, mask, formatNum, nullable, lbDate, ubDate) {
	// Boþsa null
	if ((date == null || date == "") && nullable)
		return true;
	...
	var leap = 0;
	var err = 0;
	var date_tokens = date.match(mask);

	// If the match was successful, date_tokens will not be null,
	// and the first element will be an exact match of the value
	if( date_tokens && date_tokens[0] == date)
	{
		// The values will be in the date_tokens object
		switch (formatNum) {
		case 1: // mmddyyyy
		case 4:
		case 7:
	...
	if (err == 0) {
		var dtlb = getDate(lbDate);
		var dtub = getDate(ubDate);
		var dt = new Date(year, month - 1, day); // month : zero based
		if (dt < dtlb || dt > dtub)
			err = 30;
	}
...
function CalendarPopup_CheckDate(box, mask, formatNum, nullable, dvalue, lbDate, ubDate) {
	var date = box.value;
	if (!validateDate(date, mask, formatNum, nullable, lbDate, ubDate)) {
		box.value = dvalue;
	}
}
